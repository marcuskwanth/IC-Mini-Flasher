'''
IC-Project: Mini Flasher - GUI Window
Author: Marcus KWAN
All rights reserved.
'''
...
default_cycles = 5
max_on_off_time = 100   # In 100ms
max_row = 250           # Please implement
...
# Dropdown options, lists, and dictionaries
...
def add_new_row():
    ...
    # Functions as the fixes for the scales not displaying pure integer number
    ...
    row_widgets = [
        ...
        on_time_label,
        off_time_label,
        ttk.Scale(topFrame, variable=row_vars["on_time"], from_=1, to=max_on_off_time, 
                  orient=HORIZONTAL, command=update_on_time, bootstyle="secondary"),
        ttk.Scale(topFrame, variable=row_vars["off_time"], from_=1, to=max_on_off_time, 
                  orient=HORIZONTAL, command=update_off_time, bootstyle=off_time_color),
        ttk.Button(topFrame, text=delete_button_text, width=2, command=(lambda idx=row_index: delete_row(idx)), bootstyle="danger")
    ]
    update_on_time(row_vars["on_time"].get())
    update_off_time(row_vars["off_time"].get())

    # Create widgets in a iteration manner
    ...
    # Initialize widget structures and values for the added row
    rows.append({
        "widgets": row_widgets, 
        "vars": row_vars, 
        "labels": {
    ...
    # Add tracing specifically for color changes to update style
...
def delete_row(row_index):
    if 0 <= row_index < len(rows):
        # Destroy all widgets in the deleted row
        for widget in rows[row_index]["widgets"]:
            widget.destroy()
        del rows[row_index]
        del params[row_index]
        
        # Re-grid the rows after the deleted one
...
def update_params(row_index, var_name):
    try:
        print("INFO: Updating row {0}'s parameter: {1}. No. of cycles: {2}".format(row_index, var_name, cycles.get()))
        if row_index < len(params):
            ## ERROR checking is needed (for intensity)
...
def color_enum(color):
    if color == colors[0]: return "R"
    elif color == colors[1]: return "G"
    elif color == colors[2]: return "B"
    elif color == colors[3]: return "I"
...
def send_action():
    ...
    for i in range(len(params)):
        for j in range(len(params[i-1])):
            ...
            ## ERROR checking is needed (N color)
...
def send_usb(port, baudrate, packet):
    with serial.Serial(port, baudrate, timeout=1) as ser:
        ser.write(packet)
        print("INFO: Sent {0} bytes via USB serial".format(len(packet)))
...
ttk.Button(bottomFrame, text=add_row_button_text, command=add_new_row, 
           width=15, bootstyle="primary").grid(row=0, column=0, padx=5, pady=5)
ttk.Button(bottomFrame, text=send_button_text, command=send_action, 
           width=15, bootstyle="success").grid(row=0, column=1, padx=5, pady=5)
ttk.Label(bottomFrame, text=cycles_label_text).grid(row=0, column=2, padx=(20,5), pady=5)
ttk.Spinbox(bottomFrame, from_=1, to=100, textvariable=cycles, 
